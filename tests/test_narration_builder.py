...
def test_dialogue_known_speaker_female_fr() -> None:
    ...
    Un sfx compagnon est nécessaire : la nouvelle règle n'annonce un dialogue
    que s'il reste au moins un dialogue/sfx après lui dans le même panel.
    """
...
def test_dialogue_known_speaker_ja_suffix_after() -> None:
    ...
    assert [s.kind for s in script.segments] == ["dialogue", "narration", "sfx"]
...
def test_dialogue_unknown_speaker_ja() -> None:
    ...
    assert [s.kind for s in script.segments] == ["narration", "dialogue", "sfx"]
    assert script.segments[0].text == "声が言った："
    assert script.segments[0].voice_id == VOICE_NARRATOR


def test_dialogue_alone_in_panel_gets_no_narration() -> None:
    """Un dialogue seul dans son panel (rien d'autre après) ne reçoit aucun segment narration."""
    dialogue = _make_segment("seg-1", "dialogue", "おはよう", voice_id="jf_alpha")
    script = _make_script([dialogue])

    enrich_script(script, lang="fr")

    assert script.segments == [dialogue]


def test_dialogue_last_in_panel_gets_no_narration_even_different_speaker() -> None:
    """Le dernier dialogue d'un panel n'est pas annoncé, même si le locuteur change."""
...
def test_dialogue_lookahead_stops_at_panel_boundary() -> None:
    """Un dialogue/sfx qui suit dans un AUTRE panel ne compte pas pour le lookahead."""
    dialogue = _make_segment("seg-1", "dialogue", "おはよう", voice_id="jf_alpha", panel_id="panel-a")
    other_panel_sfx = _make_segment("seg-2", "sfx", "[ドン]", panel_id="panel-b")
    script = _make_script([dialogue, other_panel_sfx])

    enrich_script(script, lang="fr")

    assert script.segments[0] is dialogue  # pas de narration insérée : le sfx est dans un autre panel
    assert [s.kind for s in script.segments] == ["dialogue", "narration", "sfx"]
...
def test_dialogue_different_speaker_gets_prefix_each_time() -> None:
    """Deux dialogues de voice_id différents, sfx en fin de panel : chacun garde son segment narration."""
    dialogue_1 = _make_segment("seg-1", "dialogue", "おはよう", voice_id="jf_alpha")
    dialogue_2 = _make_segment("seg-2", "dialogue", "元気？", voice_id="jm_kumo")
    sfx = _make_segment("seg-sfx", "sfx", "[ドン]")  # nécessaire pour que dialogue_2 ait un "après"
    script = _make_script([dialogue_1, dialogue_2, sfx])

    enrich_script(script, lang="fr")

    assert [s.kind for s in script.segments] == [
        "narration", "dialogue", "narration", "dialogue", "narration", "sfx",
    ]
    assert script.segments[0].text == "Elle dit :"
...
def test_sfx_always_announced_even_alone_in_panel() -> None:
    """Un sfx seul dans son panel (rien après) est quand même annoncé (pas de règle de suite pour sfx)."""
    sfx = _make_segment("seg-1", "sfx", "[ドン]")
    script = _make_script([sfx])
...
def test_mixed_script_preserves_order_and_inserts_only_where_needed() -> None:
    ...
    assert [s.kind for s in script.segments] == [
        ...
        "narration",  # préfixe inséré avant dialogue (sfx suit dans le même panel)
        "dialogue",
